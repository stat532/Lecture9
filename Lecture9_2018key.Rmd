...
\begin{enumerate}
	\item $\mu < \mu_0$ (one-sided test)
	\item $\mu = \mu_0$ (point test)
	\item $\mu \in [\mu_1, \mu_2]$ (interval test)
\end{enumerate}
\vfill
...
- Recall the maximum-likelihood has a related form:
	\begin{equation}
	\frac{max: \theta \in H_0 \hspace{.25cm} \mathcal{L}(\theta|\tilde{x})}{max: \theta \in H_1 \hspace{.25cm} \mathcal{L}(\theta|\tilde{x})}
	\end{equation}
...
- __Q:__  What happens as $\psi^2 \rightarrow \infty$? 
	...
	\item Consider two models: $M_1 \& M_2$, each with parameters sets $\Theta^{(M_1)}$ and $\Theta^{(M_2)}$. The Bayes Factor is:
	\begin{equation}
	...
	\vfill
- __Q:__ Can you ever specify an improper prior on any of the parameters in $\Theta^{(M_1)}$ and $\Theta^{(M_2)}$?  Yes, so long as the parameter appears in both models (and that parameter has the same meaning in both models). Otherwise, the BF is meaningless if the parameter does not appear in both models.
...
&\propto& \exp\left[-\frac{1}{2} \left(\tilde{\beta}^T \left( \Sigma_0^{-1} + X^TX / \sigma^2 \right) \tilde{\beta} - 2 \tilde{\beta} \left(\Sigma_0^{-1} \tilde{\beta}_0 + X^t \tilde{y}/\sigma^2 \right)\right)\right]
\end{eqnarray}
...
- $Var(\tilde{\beta}|\tilde{y},X,\sigma^2) = \left(\Sigma_0^{-1} + X^T X / \sigma^2 \right) ^{-1}$.
- $E[\tilde{\beta}|\tilde{y},X, \sigma^2] =  \left(\Sigma_0^{-1} + X^T X / \sigma^2 \right) ^{-1} \times \left(\Sigma_0^{-1} \tilde{\beta}_0 + X^T \tilde{y} / \sigma^2\right)$
\vfill
\newpage
...
\vfill
- Then define $\beta_i = z_i \times b_i$. Note the $b_i'$ are the real-values regression coefficients. For now we will ignore the intercept (standardizing the covariates). 
\vfill
...
\begin{eqnarray*}
E[y|\tilde{x}, \tilde{b}, \tilde{z}=(1,0,1)] &=& b_1 x_1 + b_3 x_3\\
E[y|\tilde{x}, \tilde{b}, \tilde{z}=(0,1,0)] &=& b_2 x_2.\\
\end{eqnarray*}
...
\begin{equation}
p(\tilde{y}^*|,X^*X,\tilde{y})= \sum_{i=1}^k p(\tilde{y}^*|X^*,M_k,X, \tilde{y}) Pr(M_k|X,\tilde{y}).
\end{equation}
